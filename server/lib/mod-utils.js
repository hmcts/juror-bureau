...
module.exports.paginationBuilder = function (totalResults, currentPage, url) {
  let pageItems = []
      ; let totalPages = Math.ceil(totalResults / this.constants.PAGE_SIZE)
      ; let routePart = url.split('?')[0]
...
module.exports.resolveDateFormat = function (dateStr) {
  let pattern1 = /(\d{1,2}\/)(\d{1,2}\/)(\d{4})/g
      ; let pattern2 = /(\d{4}\/)(\d{1,2}\/)(\d{1,2})/g
      ; let pattern3 = /(\d{4}-)(\d{1,2}-)(\d{1,2})/g;
