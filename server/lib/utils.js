...
const searchObj = require('../objects/search').object;
...
module.exports.searchResponses = function (req, app, searchValues) {
  return new Promise(function (resolve, reject) {
    ...
    for (iParam; iParam < paramsUsed.length; iParam += 1) {
      ...
      if (iParam > 0
        && (paramsUsed.length > 2 || multipleStatus)
        && (iParam < paramsUsed.length - 1 || (hasStatus > -1 || multipleStatus))) {
        resultsStr += ', ';
      }
...
module.exports.getResponseNameDetails = function (data) {
  let newNameRender = [data.newTitle, data.newFirstName, data.newLastName].filter(function (val) {
    return val;
  }).join(' ');

  let nameRender = [data.title, data.firstName, data.lastName].filter(function (val) {
    return val;
  }).join(' ');
