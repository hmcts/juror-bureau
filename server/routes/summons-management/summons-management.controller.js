...
module.exports.getDeferral = function (app) {
  return function (req, res) {
    ...
    let tmpFields = !!req.session.formFields ? req.session.formFields : {};
    let cancelUrl;
    let postBody = {};
    let backLinkUrl;
    let processUrl;
    let defaultDate = new Date();
    let successCB = function (poolOptions) {
...
module.exports.postDeferral = function (app) {
  return function (req, res) {
    ...
    let deferralDates = [];
    let optionSelected = [];
    let replyMethod = req.params['type'];
    let routeParameters = {
      id: req.params['id'],
      type: req.params['type'],
    };
    let sentToDeferral = req.body.sendToDeferralMaintence ? req.body.sendToDeferralMaintence : null;
...
module.exports.postExcusal = function (app) {
  return function (req, res) {
    ...
    let successCB = function () {
      let codeMessage = (code) => req.session.excusalReasons.filter((el) => el.code === code)[0].description;
      let reason = {
...
module.exports.getPaperResponseDetails = function (app) {
  return function (req, res) {
    ...
    let successCB = function (response) {
      let responseClone = _.clone(response[0].data);
      let nameDetails;
      let addressDetails;
      let jurorDetails;
      let importantNavItems;
...
function resolveJurorName (response) {
  let newNameRender = [response.title, response.firstName, response.lastName]
    .filter(function (val) {
      return val;
    }).join(' ');
  let nameRender = [response.existingTitle, response.existingFirstName, response.existingLastName]
    .filter(function (val) {
      return val;
    }).join(' ');
  let hasNewName = newNameRender !== nameRender;
...
function resolveJurorAddress (response) {
  let newAddressRender = filters.buildRecordAddress([
  ...
  let addressRender = filters.buildRecordAddress([
  ...
  let hasNewAddress = newAddressRender !== addressRender;
...
function resolveJurorDob (response) {
  let newDateOfBirth = response.dateOfBirth;
  let currentAge = moment(response.serviceStartDate, 'YYYY-MM-DD')
...
function resolveJurorEmail (response) {
  let newEmailAddress = response.emailAddress;
...
function resolveThirdParty (response) {
  let isThirdParty = response.thirdParty.relationship !== null && response.thirdParty.thirdPartyReason !== null;
...
function createDeferralDate (deferredToDate) {
  ...
  let date = new Date(intYear, intMonth - 1, intDay, 12, 0, 0);  // Date takes a 0-11 month range, set to midday
