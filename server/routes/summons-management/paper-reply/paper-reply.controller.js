(function() {
  ...
  const paperReplyObjectObj = require('../../../objects/paper-reply').paperReplyObject;
  const getJurorDetailsObj = require('../../../objects/juror-record').record;
  ...
  module.exports.hasStartedRequest = (app) => {
    return (req, res, next) => {
  ...
  module.exports.getIndex = (app) => async (req, res) => {
    ...
    if (typeof req.session.paperResponseDetails !== 'undefined'
      && req.session.paperResponseDetails.jurorNumber !== jurorNumber) {
    ...
    let response;
    try {
      response = await getJurorDetailsObj.get(
        req,
        'detail',
        jurorNumber,
        req.session.locCode
  ...
  module.exports.postIndex = (app) => (req, res) => {
    const jurorNumber = req.params['id'];
  ...
  module.exports.getEligibility = (app) => (req, res) => {
    const jurorNumber = req.params['id'];
    const tmpErrors = _.clone(req.session.errors);
  ...
  module.exports.postEligibility = (app) => (req, res) => {
  ...
  module.exports.getIneligibleAge = (app) => (req, res) => {
    const jurorNumber = req.params['id'];
    const result = getEligibilityDetails(req.session.paperResponseDetails.eligibility);
  ...
  module.exports.postIneligibleAge = (app) => async (req, res) => {
    ...
    try {
      await paperReplyObjectObj.post(req, req.session.paperResponseDetails);
  ...
  module.exports.getReplyTypes = (app) => (req, res) => {
    const jurorNumber = req.params['id'];
    let deferralValue;
  ...
  module.exports.postReplyTypes = (app) => (req, res) => {
    const jurorNumber = req.params['id'];
  ...
  module.exports.getCjsEmployment = (app) => (req, res) => {
    const jurorNumber = req.params['id'];
    const cjsEmploymentResponse =
    ...
    if (typeof req.session.paperResponseDetails.cjsEmployment !== 'undefined') {
      for (let i = 0; i < req.session.paperResponseDetails.cjsEmployment.length; i++) {
  ...
  module.exports.postCjsEmployment = (app) => (req, res) =>{
    const jurorNumber = req.params['id'];
    const cjsEmploymentResponse =
  ...
  module.exports.getAdjustments = (app) => async (req, res) => {
    const jurorNumber = req.params['id'];
    let specialNeeds = [];
    let assistanceType;
    let assistanceTypeDetails;
    let adjustmentsResponse;
    let reasons = [];
    ...
    try {
      const adjustmentsReasons = modUtils.reasonsArrToObj(await systemCodesDAO.get(req, 'REASONABLE_ADJUSTMENTS'));
  ...
  module.exports.postAdjustments = (app) => (req, res) => {
    const jurorNumber = req.params['id'];
    const specialNeeds = [];
    ...
    const validatorResult = validate(req.body, paperReplyValidator.reasonableAdjustments());
    if (typeof validatorResult !== 'undefined') {
      req.session.errors = validatorResult;
      req.session.formFields = req.body;
  ...
  module.exports.getSignature = (app) => (req, res) => {
    const jurorNumber = req.params['id'];
    const signed = (req.session.paperResponseDetails && req.session.paperResponseDetails.signed !== 'undefined')
        ? req.session.paperResponseDetails.signed : false;
    const tmpError = _.clone(req.session.error);
  ...
  module.exports.postSignature = (app) => async (req, res) => {
    const jurorNumber = req.params['id'];
    ...
    try {
      const paperSummonsData = await paperReplyObjectObj.post(
        req,
  ...
  module.exports.getStraightThrough = (app) => (req, res) => {
    const jurorNumber = req.params['id'];
  ...
  module.exports.postStraightThrough = (app) => async (req, res) => {
    const jurorNumber = req.params['id'];

    if (req.body.response !== jurorNumber) {
      app.logger.crit('Response id and url parameters do not match: ', {
        auth: req.session.authentication,
        data: {
          urlResponseId: jurorNumber,
  ...
  module.exports.getEditName = (app) => (req, res) => {
    ...
    const jurorDetails = {};
    const tmpErrors = _.clone(req.session.errors);
    const tmpData = _.clone(req.session.formFields) || {};
  ...
  module.exports.postEditAddress = (app) => (req, res) => {
    const jurorNumber = req.params['id'];

    const validatorResult = validate(req.body, paperReplyValidator.jurorAddress());
    if (typeof validatorResult !== 'undefined') {
      req.session.errors = validatorResult;
      req.session.formFields = req.body;
  ...
  const getEligibilityDetails = (eligibilityData) => {
    const eligibility = {
      livedConsecutive: '',
      mentalHealthAct : '',
  ...
  const createEligibilityObject = (body) => {
  ...
  const mergeMentalHealthInfo = (eligibilityPayload) => {
