...
module.exports.post = function (app) {
  return function (req, res) {
    ...
    let modAccess = req.session.hasModAccess;

    let validatorResult;

    const updateSuccess = function (apiResponse) {
    ...
    const updateError = function (apiResponse) {
    ...
    const readSuccess = function (apiResponse) {
      app.logger.info('Fetched backlog information: ', {
        auth: req.session.authentication,
        jwt: req.session.authToken,
        response: apiResponse,
