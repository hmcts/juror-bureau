(function() {
  ...
  module.exports.postEditApprovalExpenses = function(app) {
    return async function(req, res) {
      ...
      try {
        await postEditedExpensesDAO.put(req, locCode, jurorNumber, STATUSES[status], expensesToPost);
  ...
  module.exports.postEditApprovalExpensesEditApplyToAll = function(app) {
    return async function(req, res) {
      ...
      if (req.body.applyToAllDays.includes('paymentMethod')) {
        applyToAllPayload['paymentMethod'] = req.body.paymentMethod;
      }
      ...
      try {
        ...
        if (status !== 'for-approval') {
          ...
          for (const expense of expensesData) {
            ...
            if (req.body.applyToAllDays.includes('otherCosts') && applyToAllPayload.financialLoss) {
              if (applyToAllPayload.financialLoss.otherCosts
                && parseFloat(applyToAllPayload.financialLoss.otherCosts) < expense.financialLoss.otherCosts) {
                errors.otherCosts = [{
        ...
        for (const expense of expensesData) {
          ...
          const expenseTableValues = _.clone(recalculateValues.find(expense => expense['attendanceDate'] === expenseDate));
  ...
  async function isLossOverLimit(app, req, attendanceType, date) {
    ...
    if (totalFinancialLoss > lossLimit) {
      ...
      if (req.session.editedExpenses[date] && req.session.editedExpenses[date].formData) {
        const fin = _.clone(req.session.editedExpenses[date].formData['financialLoss']);
        let effectiveLossOfEarnings = Number(fin['lossOfEarnings']) || 0;
        let effectiveExtraCareCost = Number(fin['extraCareCosts']) || 0;
        let effectiveOtherCost = Number(fin['otherCosts']) || 0;
        ...
        if (total > lossLimit) {
          ...
          req.session.editedExpenses[date].formData['financialLoss']['lossOfEarnings'] = effectiveLossOfEarnings;
          req.session.editedExpenses[date].formData['financialLoss']['extraCareCosts'] = effectiveExtraCareCost;
          req.session.editedExpenses[date].formData['financialLoss']['otherCosts'] = effectiveOtherCost;
        }
      }
