...
module.exports.postFilterJurors = function (app) {
  return function (req, res) {
    ...
    if (_filters.jurorNumber) {
      queryParams.set('jurorNumber', _filters.jurorNumber);
    } else {
      queryParams.delete('jurorNumber');
    }
    if (_filters.firstName) {
      queryParams.set('firstName', _filters.firstName);
    } else {
      queryParams.delete('firstName');
    }
    if (_filters.lastName) {
      queryParams.set('lastName', _filters.lastName);
    } else {
      queryParams.delete('lastName');
    }
    if (_filters.attendance) {
      queryParams.set('attendance', _filters.attendance);
    } else {
      queryParams.delete('attendance');
    }
    if (_filters.checkedIn === 'today') {
      queryParams.set('checkedIn', 'true');
    } else {
      queryParams.delete('checkedIn');
    }
    if (_filters.nextDueAtCourt) {
      queryParams.set('nextDue', _filters.nextDueAtCourt);
    } else {
      queryParams.delete('nextDue');
    }
    if (_filters.status) {
      queryParams.set('status', _filters.status);
    } else {
...
module.exports.getHistory = function (app) {
  return function (req, res) {
    let poolNumber = req.params['poolNumber'];
    let coronerPoolPrefix = '9' + new Date().getFullYear().toString().slice(2, 4);
...
async function courtView (app, req, res, pool, membersList, _errors, selectedJurors, selectAll) {
  ...
  const assignUrl = app.namedRoutes.build('pool-overview.reassign.post',
    { poolNumber: req.params.poolNumber });
  const transferUrl = app.namedRoutes.build('pool-overview.transfer.post',
    { poolNumber: req.params.poolNumber });
  const completeServiceUrl = app.namedRoutes.build('pool-overview.complete-service.post',
    { poolNumber: req.params.poolNumber });
  const changeServiceDateUrl = app.namedRoutes.build('pool-overview.change-next-due-at-court.post',
    { poolNumber: req.params.poolNumber });
  const postponeUrl = app.namedRoutes.build('pool-overview.postpone.post',
    { poolNumber: req.params.poolNumber });
