...
module.exports.postJurorsList = function(app) {
  return async function(req, res) {
    ...
    try {
      const response = modUtils.mapSnakeToCamel(await jurorAttendanceDao.post(req, payload));

      notCheckedOut = response.details.filter((juror) => selectedJurorNo.includes(juror.jurorNumber));
    } catch (err) {
...
function sortJurors(req) {
  const SORT_BY = {
    ...
    nextDueAtCourt: 'nextDueAtCourt',
    serviceStartDate: 'serviceStartDate',
  };

  const { sortBy, sortDirection } = req.query;
  const _sortBy = SORT_BY[sortBy] || 'jurorNumber';
...
function sortPools(req, pools) {
  ...
  const _sortBy = SORT_BY[sortBy] || 'poolNumber';
