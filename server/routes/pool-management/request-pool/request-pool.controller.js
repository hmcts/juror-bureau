...
const requestPoolObj = require('../../../objects/request-pool');
...
module.exports.postPoolDetails = function (app) {
  return function (req, res) {
    let validatorResult;
    let tmpPoolDetails = req.session.poolDetails;
...
module.exports.getChangeAttendanceDate = function (app) {
  return function (req, res) {
    let tmpErrors;
    let tmpFormFields;
    let currentSetDate = new Date(req.session.poolDetails.attendanceDate || null);
...
function poolNumberPrefixBuilder (poolDetails) {
  let courtCode = poolDetails.courtCode;
  let dateJoin = dateFilter(poolDetails.attendanceDate, null, 'YYMM');
