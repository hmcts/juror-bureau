...
const fetchCoronerPoolObj = require('../../../objects/request-pool').fetchCoronerPool;

module.exports.getSelectCourt = function (app) {
  return function (req, res) {
    let transformedCourtNames;
    let tmpErrors = _.clone(req.session.errors);
    let tmpData = _.clone(req.session.coronerCourt) || {};
...
module.exports.postSelectCourt = function (app) {
  return function (req, res) {
    const successCB = function (court) {
    ...
    const validatorResult = validate(req.body, selectCourtValidator());
...
module.exports.getPoolDetails = function (app) {
  return function (req, res) {
    ...
    const requestedDate = (typeof req.session.coronerCourt.requestedDate === 'undefined'
...
module.exports.postPoolDetails = function (app) {
  return function (req, res) {
    ...
    const validatorResult = validate(req.body, poolDetailsValidator());
    if (typeof validatorResult !== 'undefined') {
      req.session.errors = validatorResult;
      req.session.formFields = req.body;
...
module.exports.getCheckDetails = function (app) {
  return function (req, res) {
    const coronerCourtPoolFailed = req.session.coronerCourtPoolFailed;
...
module.exports.postCheckDetails = function (app) {
  return function (req, res) {
    const successCB = function (response) {
      const poolNumber = new URLSearchParams(response.headers.location).get('poolNumber');
...
module.exports.getCatchmentArea = function () {
  return function (req, res) {
    const poolDetails = _.clone(req.session.coronerCourt);
...
module.exports.getPostCodes = function (app) {
  return function (req, res) {
    ...
    const successCB = function (response) {
      let postcodes = [];
      let addCitizensError = req.session.addCitizensError;
      let tmpErrors = _.clone(req.session.errors);
      let fieldErrorMessages = {};
      let maxExceededError = {};
...
module.exports.getExportPool = function (app) {
  return function (req, res) {
    const successCB = function (data) {
      let csvResult;
      let filename;
      let requestDate = dateFilter(new Date(data.dateRequested), null, 'DD/MM/YYYY');
