...
const nilPoolCheck = require('../../../objects/nil-pool').nilPoolCheck;
const nilPoolCreate = require('../../../objects/nil-pool').nilPoolCreate;
const checkDayType = require('../check-day-type');
let courtsList; // TODO: make sure this does not conflict with any other courtsList set around this file
...
module.exports.post = function (app) {
  return function (req, res) {
    ...
    const validatorResult = validate(req.body, require('../../../config/validation/request-pool').poolType(req));
...
module.exports.getChangeCourt = function (app) {
  return function (req, res) {
    ...
    const transformedCourtNames = modUtils.transformCourtNames(courtsList);
...
module.exports.postChangeCourt = function (app) {
  return function (req, res) {
    ...
    const validatorResult = validate(req.body,
      require('../../../config/validation/request-pool').courtNameOrLocation(req));
    if (typeof validatorResult !== 'undefined') {
      req.session.errors = validatorResult;
      req.session.formFields = req.body;
...
module.exports.getChangeAttendanceDate = function () {
  return function (req, res) {
    // if we post an invalid date and go back,
    // we then want to remove the tmpDate and show the old date
    delete req.session.tmpDate;

    const selectedAttendanceDate = new Date(req.session.poolDetails.attendanceDate);
...
module.exports.postChangeAttendanceDate = function (app) {
  return function (req, res) {
    ...
    const validatorResult = validate(req.body, require('../../../config/validation/request-pool.js').validateDate());
    if (typeof validatorResult !== 'undefined') {
      req.session.errors = validatorResult;
      req.session.formFields = req.body;
...
module.exports.getInvalidDate = function () {
  return function (req, res) {
    let invalidDate = {};
...
module.exports.getCheckNilPoolDetails = function () {
  return function (req, res) {
    const tmpBody = _.clone(req.session.poolDetails);
