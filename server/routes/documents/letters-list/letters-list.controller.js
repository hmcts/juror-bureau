(function() {
  ...
  module.exports.getListLetters = function(app) {
    return function(req, res) {
      try {
        ...
        if (documentSearchBy === 'jurorNumber') {
          searchBy = jurorNumber;
        } else if (documentSearchBy === 'jurorName') {
          searchBy = jurorName;
  ...
  module.exports.postListLetters = function(app) {
    return async function(req, res) {
      ...
      const checkedJurors = _.cloneDeep(req.session.documentsJurorsList.checkedJurors);

      checkedJurors.forEach((juror) => {
        if (juror.datePrinted === 'null') {
          juror.datePrinted = null;
        }
      });

      const payload = {
        letters: checkedJurors,
      };
